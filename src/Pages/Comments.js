...
export default function Comments() {
  // const { user, setUser } = useContext(AppContext);ö
  ...
  const handleGet = async (id) => {
    const res = await axios.get(
      `${process.env.REACT_APP_API_KEY}basic/web/index.php/comments/${id}`
    );
    setValues(res.data);
  };
  ...
  const handleComment = (e) => {
    e.preventDefault();
    const url = `${process.env.REACT_APP_API_KEY}basic/web/index.php/comments`;
